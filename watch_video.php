...
if(isset($_POST['like_content'])){
   ...
   if($user_id != ''){
      ...
      if($select_likes->rowCount() > 0){
         ...
         $message[] = 'Lección removida de tu lista de likes';
      }else{
         ...
         $message[] = 'Lección añadida a tu lista de likes';
      }
   ...
   if($user_id != ''){
      ...
      if($select_content->rowCount() > 0){
         ...
         if($select_comment->rowCount() > 0){
            $message[] = 'Ya se agrego tu comentario';
         }else{
            ...
            $message[] = 'Nuevo comentario subido!';
         }
...
<section class="watch-video">
   ...
   <?php
      ...
      if($select_content->rowCount() > 0){
         while($fetch_content = $select_content->fetch(PDO::FETCH_ASSOC)){
            ...
            $selectLections = $conn->prepare("SELECT * FROM `preguntas` WHERE id = ?");
            $selectLections->execute([$get_id]);
            $total_quiz = $selectLections->rowCount();

            $vidSOURCE = $fetch_content['video'];
   ?>
   <div class="video-details">
      ...
      <form action="" method="post" class="flex">
         ...
         <a href="playlist.php?get_id=<?= $fetch_content['playlist_id']; ?>" class="inline-btn">Volver a clase</a>
         <?php
            if($verify_likes->rowCount() > 0){
         ?>
         <button type="submit" name="like_content"><i class="fas fa-heart"></i><span>En likes</span></button>
         <?php
         }else{
         ?>
      ...
      if ($total_quiz>0) {
      ?>
      <a href="answquiz.php?get_id=<?= $content_id; ?>" class="btn">Contestar Ejercicios</a>
      <?php
      }
      ?>
   </div>
   ...
   <?php
         ...
         echo '<p class="empty">No se encontro la lección</p>';
      }
   ?>
...
<section class="comments">
   ...
   <h1 class="heading">Hacer un comentario</h1>

   <form action="" method="post" class="add-comment">
      <input type="hidden" name="content_id" value="<?= $get_id; ?>">
      <textarea name="comment_box" required placeholder="Escribe tu mensaje..." maxlength="1000" cols="30" rows="10"></textarea>
      <input type="submit" value="Comentar" name="add_comment" class="inline-btn">
   </form>

   <h1 class="heading">Comentarios de otros</h1>
   ...
   <div class="show-comments">
      ...
      <?php
         ...
         echo '<p class="empty">No se encontraron comentarios, se el primero en hacerlo!</p>';
      }
      ?>
      </div>
