...
<section class="playlist">
   ...
   <h1 class="heading">Detalles de la clase</h1>
   ...
   <div class="row">
      ...
      <?php
         ...
         if($select_playlist->rowCount() > 0){
            ...
            $count_lections = $conn->prepare("SELECT * FROM `content` WHERE playlist_id = ?");
            $count_lections->execute([$playlist_id]);
            $total_lections = $count_lections->rowCount();
      ...
      <div class="col">
         <form action="" method="post" class="save-list">
            ...
            ?>
            <button type="submit" name="save_list"><i class="fas fa-bookmark"></i><span>Clase guardada</span></button>
            <?php
               }else{
            ?>
               <button type="submit" name="save_list"><i class="far fa-bookmark"></i><span>Guardar esta clase</span></button>
            <?php
               }
            ?>
         </form>
         <div class="thumb">
            <span><?= $total_lections; ?> Lecciones</span>
      ...
      <?php
            ...
            echo '<p class="empty">no se encontro la clase</p>';
         }  
      ?>
...
<section class="videos-container">
   ...
   <h1 class="heading">Lecciones en esta clase</h1>
      ...
      <?php
         ...
         if($select_content->rowCount() > 0){
            while($fetch_content = $select_content->fetch(PDO::FETCH_ASSOC)){  
               $thumbSOURCE = $fetch_content['thumb'];
            if ($thumbSOURCE=="") {
               $thumbSOURCE="udg.png";
            }
      ?>
      <a href="watch_video.php?get_id=<?= $fetch_content['id']; ?>" class="box">
         <img src="uploaded_files/<?=  $thumbSOURCE; ?>" alt="">
         <h3><?= $fetch_content['title']; ?></h3>
      </a>
      <?php
            }
         }else{
            echo '<p class="empty">No hay lecciones disponibles</p>';
         }
      ?>
